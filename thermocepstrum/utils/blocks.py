...
def main():
     usage="""usage: {} output N_currents N_processes DT_FS max_THz_psd input1 input2 ... inputN
...
#     print selection_not_zero
     ...
     with PdfPages(output+"_all.pdf") as pdf:
         ...
         plt_psd(psd,k_00=True,f_THz_max=max_THz)
         pdf.savefig()
         plt.close()
...
def plt_psd(jf,j2=None,j2pl=None,f_THz_max=None, k_SI_max=None,k_00=False):
    ...
    try:
       plt.plot(jf.freqs_THz,np.real(jf.cospectrum[0,0])*jf.kappa_scale*.5,c=c[2],lw=1.0,)
    except AttributeError:
       pass

...
def main():
     usage="""usage: {} output N_currents N_processes DT_FS max_THz_psd nyquist k_SI_max input1 input2 ... inputN
     ...
     if sys.argv[7]=='None' or sys.argv[7]=='none' or sys.argv[7]=='0':
         k_SI_max=None
     else:
     ...
     log.write( '\n')
     log.write( 'Mean value and standard deviation of kappa(aic_Kminm):                   {} +/- {}\n'.format(np.mean(cepstrals[:,0,aic_Kmin]),np.std(cepstrals[:,0,aic_Kmin])))
     log.write( 'Mean value of calculated statistical error per block of kappa(aic_Kmin): {}\n'.format(np.mean(cepstrals[:,1,aic_Kmin])))
     log.write( '\n')
     log.write( 'Mean value and standard deviation of L0(aic_Kmin):                       {} +/- {}\n'.format(np.mean(cepstrals[:,2,aic_Kmin]),np.std(cepstrals[:,2,aic_Kmin])))
     log.write( 'Mean value of calculated statistical error per block of L0(aic_Kmin):    {}\n'.format(np.mean(cepstrals[:,3,aic_Kmin])))
     log.close()
...
def plt_psd(jf,j2=None,j2pl=None,f_THz_max=None, k_SI_max=None,k_00=False,nyq=None):
    ...
    if k_SI_max==None:
       ...
       if k_00 :
         try:
           k_SI_max2=np.max(np.real(jf.cospectrum[0,0][:int(jf.freqs_THz.shape[0]*f_THz_max/jf.freqs_THz[-1])])*jf.kappa_scale*.5)*1.3
    ...
    if j2pl != None:
       plt.plot(j2pl.freqs_THz,j2pl.dct.psd*j2pl.kappa_scale*.5,c=c[2])
    try:
       plt.plot(jf.freqs_THz,np.real(jf.cospectrum[0,0])*jf.kappa_scale*.5,c=c[3],lw=1.0,zorder=1)
    except (AttributeError, TypeError) as e:
       pass
  ...
  #  plt.ylabel('$^{\ell M}\widehat{S}\'_{\,k}$ (W/mK)')
    plt.ylabel('(W/mK)')
    idxnyq=int(nyq/jf.freqs_THz[-1]*jf.freqs_THz.size-1)
    if nyq != None:
       plt.annotate("", xy=(nyq, (k_SI_max-jf.fpsd[idxnyq]*jf.kappa_scale*.5)/7+jf.fpsd[idxnyq]*jf.kappa_scale*.5), \
